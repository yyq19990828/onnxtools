...
class TestAnnotatorPipelineIntegration:
    ...
    def test_simple_two_annotator_pipeline(self, test_image, test_detections):
        ...
        pipeline = AnnotatorPipeline().add(AnnotatorType.BOX, {"thickness": 2}).add(AnnotatorType.RICH_LABEL, {})
    ...
    def test_rendering_order_matters(self, test_image, test_detections):
        ...
        pipeline1 = AnnotatorPipeline().add(AnnotatorType.BOX, {"thickness": 2}).add(AnnotatorType.DOT, {"radius": 8})

        # Pipeline 2: Dot then Box
        pipeline2 = AnnotatorPipeline().add(AnnotatorType.DOT, {"radius": 8}).add(AnnotatorType.BOX, {"thickness": 2})
    ...
    def test_conflict_warning_generation(self):
        """Test that conflicting combinations are rejected at add time."""
        # BOX + ROUND_BOX should raise ValueError due to conflict
    ...
    def test_pipeline_with_privacy_and_labels(self, test_image, test_detections):
        ...
        pipeline = (
            AnnotatorPipeline().add(AnnotatorType.BOX, {"thickness": 2}).add(AnnotatorType.BLUR, {"kernel_size": 15})
        )
    ...
    def test_pipeline_with_different_images(self, test_detections):
        ...
        pipeline = AnnotatorPipeline().add(AnnotatorType.BOX, {}).add(AnnotatorType.DOT, {})
