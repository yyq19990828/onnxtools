...
class TestDotAnnotatorIntegration:
    ...
    def test_dot_basic_rendering(self, test_image, test_detections):
        ...
        annotator = AnnotatorFactory.create(AnnotatorType.DOT, {"radius": 5, "position": sv.Position.CENTER})
    ...
    def test_dot_different_radii(self, test_image, test_detections):
        ...
        for radius in [3, 5, 8, 10]:
            annotator = AnnotatorFactory.create(AnnotatorType.DOT, {"radius": radius, "position": sv.Position.CENTER})
    ...
    def test_dot_different_positions(self, test_image, test_detections):
        ...
        positions = [sv.Position.CENTER, sv.Position.TOP_LEFT, sv.Position.BOTTOM_RIGHT]
        for position in positions:
            annotator = AnnotatorFactory.create(AnnotatorType.DOT, {"radius": 5, "position": position})
            result = annotator.annotate(test_image.copy(), test_detections)
            assert result.shape == test_image.shape
    ...
    def test_dot_with_outline(self, test_image, test_detections):
        ...
        annotator = AnnotatorFactory.create(
            AnnotatorType.DOT,
            {"radius": 5, "position": sv.Position.CENTER, "outline_thickness": 2, "outline_color": sv.Color.BLACK},
        )
        result = annotator.annotate(test_image, test_detections)
        assert result.shape == test_image.shape
