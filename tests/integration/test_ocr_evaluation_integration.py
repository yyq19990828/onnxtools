...
class TestOCRDatasetEvaluatorIntegration:
    ...
    def test_end_to_end_evaluation_json_format(self, sample_dataset, ocr_model_path, ocr_character, capsys):
        ...
        results = evaluator.evaluate_dataset(
            label_file=str(label_file), dataset_base_path=str(dataset_base), conf_threshold=0.3, output_format="json"
        )
    ...
    def test_evaluation_with_missing_images(self, tmp_path, ocr_model_path, ocr_character):
        ...
        # Run evaluation
        results = evaluator.evaluate_dataset(
            label_file=str(label_file), dataset_base_path=str(dataset_path), conf_threshold=0.5, output_format="table"
        )
    ...
    @pytest.mark.skipif(not Path("data/ocr_rec_dataset_examples").exists(), reason="Real OCR dataset not available")
