...
class TestNonMaxSuppression:
    ...
    def _make_prediction(self, boxes_xywh, class_scores, batch=True):
        """Helper: create prediction array in modern YOLO format [batch, anchors, 4+num_classes]."""
        # boxes_xywh: (N, 4), class_scores: (N, C)
    ...
    def test_empty_batch(self):
        # Prediction with no anchors above threshold
    ...
    def test_single_detection_modern_yolo(self):
    ...
    def test_max_det_limit(self):
        ...
        n = 500
        boxes = np.random.rand(n, 4).astype(np.float32) * 600
        boxes[:, 2:] = 50  # uniform size
        scores = np.random.rand(n, 2).astype(np.float32)
    ...
    def test_legacy_yolo_with_objectness(self):
