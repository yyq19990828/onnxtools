...
class Detector:
    ...
    def __init__(
        ...
        self.tracker: Optional[BaseTracker] = (
            create_tracker(tracker_algo, **tracker_kwargs) if enable_tracking else None
        )
...
def process_video(det, source, output_dir, output_mode, frame_skip, save_json):
    ...
    if not cap.isOpened():
        ...
        return
    source_name = (
        'camera' if str(source).isdigit()
        else 'rtsp' if str(source).startswith('rtsp://')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enable-tracking', action='store_true',
                        help='Enable 2D tracking (video / camera only)')
